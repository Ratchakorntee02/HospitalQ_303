...
function Navbar({ currentPage, language, session, onLanguageChange, onLogout, onNavigate, onSearch }) {
  ...
  const goDefaultHome = () => {
    onNavigate(isDoctor ? 'doctor-dashboard' : 'home');
  };
        ...
        <div className="collapse navbar-collapse" id="navbarSupportedContent">
          <ul className="navbar-nav me-auto mb-2 mb-lg-0">
              ...
              <>
                ...
                <li className="nav-item dropdown">
                  ...
                  <ul className="dropdown-menu" style={{ backgroundColor: '#5884b1' }} aria-labelledby="navbarDropdown">
                    <li>
                      <button className="dropdown-item text-white" type="button" onClick={() => onNavigate('annual-check')}>
                        {text.annual}
                      </button>
                    </li>
                    <li>
                      <button className="dropdown-item text-white" type="button" onClick={() => onNavigate('telemedicine')}>
                        {text.telemedicine}
                      </button>
                    </li>
                    <li><hr className="dropdown-divider" /></li>
                    <li>
                      <button className="dropdown-item text-white" type="button" onClick={() => onNavigate('contact')}>
                        {text.contact}
                      </button>
                    </li>
                  </ul>
            ...
            {session && (
              <li className="nav-item">
                <button
                  className={`nav-link nav-button text-white ${isActive('profile') ? 'active fw-bold' : ''}`}
                  type="button"
          ...
          <form className="d-flex align-items-center" onSubmit={handleSearchSubmit}>
            ...
            <div className="btn-group me-2" role="group" aria-label={text.language}>
              <button className={`btn btn-light btn-sm ${language === 'en' ? 'active' : ''}`} type="button" onClick={() => onLanguageChange('en')}>EN</button>
              <button className={`btn btn-light btn-sm ${language === 'th' ? 'active' : ''}`} type="button" onClick={() => onLanguageChange('th')}>TH</button>
            </div>
            <button
              className={`btn navbar-auth-btn ${session ? 'btn-danger' : 'btn-success'}`}
              type="button"
